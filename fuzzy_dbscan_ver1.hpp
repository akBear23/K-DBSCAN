...
vector<int> computeNeighbors(vector<vector<double>> data, int point, double eps) {
    ...
    for (int i = 0; i < data.size(); i++){
        ...
        if (distance(data[i], data[point]) <= eps) {
...
void expandFuzzyCluster(vector<vector<double>> data, int point, vector<int> neighbors, double eps, int minPtsMin, int minPtsMax, vector<bool>& visited, vector<vector<double>>& memberships, int& currentCluster) {
