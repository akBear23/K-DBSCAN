...
ostream& operator<<(ostream& cout, ObjWithoutCoord ele) {
	if (ele.instance >= 10)
		cout << ele.feature << "." << (ele.instance);
	else
		cout << ele.feature << ".0" << (ele.instance);
	return cout;
}
...
int pr = 0; // 1 for cout, 0 for not
int cl = 1; // 1 for cout, 0 for not
...
* @retval: SN: a hash map that stores as <instance, <<neighbors>,<neighbors>>. This struture is different with star neighbors in Join-less
*/
std::pair<std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>>, std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>>> genStarNeighborhoods1(std::map<std::pair<int, int>, std::vector<ObjWithCoord>> grid, float dist_thres)
{
	std::pair<std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>>, std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>>> SN;
	...
	for (std::map<std::pair<int, int>, std::vector<ObjWithCoord>>::iterator it = grid.begin(); it != grid.end(); ++it)
	{
		...
		fiveCells.push_back(it->first); // make_pair(i, j);
		fiveCells.push_back(make_pair(i, j + 1));
		fiveCells.push_back(make_pair(i + 1, j + 1));
		fiveCells.push_back(make_pair(i + 1, j));
		fiveCells.push_back(make_pair(i + 1, j - 1));
			...
			for (auto const& checkInst : fiveCellInst) // check with each instance in the five cells
			{
				if (currentInst.feature != checkInst.feature) // only check two instances belong to different features.
				{
					...
					if (dist <= dist_thres) // the two instances have neighbor relationship
					{
						...
						if (SN.first.find(key) == SN.first.end()) SN.first.insert({ key, std::set<ObjWithoutCoord>{} });
						if (SN.first.find(starN) == SN.first.end()) SN.first.insert({ starN, std::set<ObjWithoutCoord> {} });
						if (SN.second.find(key) == SN.second.end()) SN.second.insert({ key, std::set<ObjWithoutCoord>{} });
						if (SN.second.find(starN) == SN.second.end()) SN.second.insert({ starN, std::set<ObjWithoutCoord> {} });

						if (key < starN) {
							SN.second[key].insert(starN);
							SN.first[starN].insert(key);
						}
						else {
							SN.second[starN].insert(key);
							SN.first[key].insert(starN);
						}
					}
				}
			}
...
*/
void generateCliques_IDS(std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>> SN)
{
		...
		while (!Q.empty())
		{
			...
			if (childrenNodes.empty()) // The currNode is a leaf node, it has no children
			{
				...
				if (onecl.size() > 1)
				{
					if (cl == 1) {
						std::cout << "One clique: ";
						for (auto const& ele : onecl) std::cout << (ele) << ", ";
						std::cout << endl;
					}
...
* @retval: CLs: a vector of all cliques
*/
void generateCliques2_IDS(std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>> SN)
{
	// 1. Create a vector to store all cliques	
	std::set<ObjWithoutCoord> onecl;
	// 2. Create an root
...
/**
* @brief: This function generates all cliques.
* @param: BNs, SNs: star neighbors
* @retval: CLs: a vector of all cliques
*/
void generateCliques_NDS(std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>> SNs, std::map<ObjWithoutCoord, std::set<ObjWithoutCoord>> BNs) {
	
	// Initial the tree
	tree<ObjWithoutCoord> NTree;
	std::map<ObjWithoutCoord, tree<ObjWithoutCoord>::iterator> headList;

	// Initiate variables for while loop
	tree<ObjWithoutCoord>::iterator root = NTree.begin(), headNode; // Traverse the tree
	tree<ObjWithoutCoord>::iterator sib2, end2; // Get all siblings of a node

	std::vector<ObjWithoutCoord> tmp; // Get one body (getBodies)
	std::map<vector<ObjWithoutCoord>, tree<ObjWithoutCoord>::leaf_iterator> bodies; // Get all the bodies of one headNode

	std::vector<ObjWithoutCoord> relation; // Get relation
	std::vector<ObjWithoutCoord> Q; // The queue of instance
	std::set<ObjWithoutCoord> onecl; // Store one clique for C Hashing

	// Loop each instance in nbs
	for (auto const& s : BNs) { // s = [key, value]

		for (auto const& sns : SNs[s.first]) {
			Q.push_back(sns);
		}
		...
		while (!Q.empty()) {
			...
			// Print instance
			if (pr == 1) std::cout << "- Current instance: " << s.first << endl;

			// Get headNode
			ObjWithoutCoord curr = Q[0]; Q.erase(Q.begin()); // queue.out

			// Print current queue
			if (pr == 1) {
				std::cout << "Current queue: ";
				for (auto const& ele : Q) std::cout << ele << ", ";
				std::cout << endl;
			}
			
			if (headList.find(curr) == headList.end()) { // headNode not existed
				headNode = NTree.insert(root, curr); // insert headNode to NTree
				headList[curr] = headNode; // add headNode to headList
			}
			else {
				headNode = headList[curr];
			}

			// Print headNode
			if (pr == 1) std::cout << "Current headNode: " << (headNode.node->data) << " \n";

			// Get all bodies of headNode
			if (pr == 1) std::cout << "All bodies: ";
			tree<ObjWithoutCoord>::leaf_iterator leaf = NTree.begin_leaf(headNode);
			while (leaf != NTree.end_leaf(headNode)) {
				tree<ObjWithoutCoord>::iterator cur = leaf; // traverse from the bottom
				while (cur != headNode) {
				...
				if (!tmp.empty()) {
					if (pr == 1) {
						std::cout << "(";
						for (auto const& ele : tmp) std::cout << (ele) << ", ";
						std::cout << "), ";
					}
					bodies[tmp] = leaf;
				}
			...
			if (pr == 1) std::cout << endl;

			// relation = intersection(queue, BNs(headNode))
			std::set_intersection(Q.begin(), Q.end(), BNs[curr].begin(), BNs[curr].end(), back_inserter(relation));
			if (pr == 1) {
				std::cout << "BNs: ";
				for (auto const& ele : BNs[curr]) std::cout << ele << ", ";
				std::cout << endl;
			}
			...
			if (pr == 1) {
				std::cout << "Relation: ";
				for (auto const& ele : relation) std::cout << (ele) << ", ";
				std::cout << endl;
			}

			// if bodies == null || relation == null
			if (relation.empty() || bodies.empty()) {
				if (pr == 1) std::cout << "-> Method 0\n";
				NTree.append_child(headNode, s.first); // headNode.addNode(s)
			}
			else {
				for (auto l = bodies.begin(); l != bodies.end(); l++) {

					tree<ObjWithoutCoord>::sibling_iterator sib = headNode.begin(), parent;

					// tmp = intersection(relation, l)
					std::set_intersection(relation.begin(), relation.end(), l->first.begin(), l->first.end(), back_inserter(tmp));

					if (l->first == relation) { // l = relation
						if (pr == 1) std::cout << "-> Method 1\n";
						NTree.append_child(l->second, s.first);
					}
					else if (std::includes(l->first.begin(), l->first.end(), relation.begin(), relation.end())) { // relation is subset of l
						if (pr == 1) {
							// Print tmp
						...
						// Insert intersection into tree
						tree<ObjWithoutCoord>::iterator cur = headNode;
						std::vector<ObjWithoutCoord>::iterator cur_tmp = tmp.begin();
						tree<ObjWithoutCoord>::sibling_iterator sibl;
						while (cur_tmp != tmp.end()) {
							sibl = cur.begin();
							if (sibl == nullptr) {
								cur = NTree.append_child(cur, *cur_tmp);
								break;
							}
							else {
								while (sibl->feature != cur_tmp->feature || sibl->instance != cur_tmp->instance) {
									++sibl;
									if (sibl == cur.end()) {
										cur = NTree.append_child(cur, *cur_tmp);
										break;
						...
						// Add s.first into tree
						sibl = cur.begin();
						if (sibl == nullptr) {
							NTree.append_child(cur, s.first);
						}
						else {
							while (sibl->feature != s.first.feature || sibl->instance != s.first.instance) {
								++sibl;
								if (sibl == cur.end()) {
									NTree.append_child(cur, s.first);
									break;
					...
					else if (std::includes(relation.begin(), relation.end(), l->first.begin(), l->first.end())) { // l is subset of relation
						if (pr == 1) {
						...
						// Add s.first into tree
						tree<ObjWithoutCoord>::sibling_iterator sibl = l->second.begin();
						if (sibl == nullptr) {
							NTree.append_child(l->second, s.first);
						}
						else {
					...
					else if (tmp.size() > 0) { // relation intersect l not empty
					...
					/*else {
						tree<ObjWithoutCoord>::sibling_iterator sibl = headNode.begin();
						if (sibl == nullptr) {
							NTree.append_child(headNode, s.first);
						}
						else {
							while (sibl->feature != s.first.feature || sibl->instance != s.first.instance) {
								++sibl;
								if (sibl == headNode.end()) {
									NTree.append_child(headNode, s.first);
									break;
								}
							}
			...
			// Clear all the vector for the iterator
			bodies.clear();
			relation.clear();
			tmp.clear();
			...
			if (pr == 1) {
				...
				tree<ObjWithoutCoord>::iterator cur = NTree.begin();
				while (cur != NTree.end()) {
					for (int i = 0; i < NTree.depth(cur); ++i) std::cout << "  ";
					std::cout << cur.node->data << endl;
					cur++;
				}
				std::cout << endl;
			}
	...
	// Get all cliques from tree, put it into C hash
	tree<ObjWithoutCoord>::leaf_iterator leaf = NTree.begin_leaf(), cur;
	while (leaf != NTree.end_leaf()) {
		cur = leaf;
		while (cur.node) {
			onecl.insert(*cur);
			cur = cur.node->parent;
...
std::map<std::string, float> CalPIandFilterPatterns2(float prev_thres)
{
	...
	while (!candidates.empty())
	{
		// 4.0 Count the number of candidates
		++numOfCands;
		...
		if (PI >= prev_thres)
		{
			...
			for (size_t t = 2; t < currCandidate.size(); ++t)
			{
				...
				for (auto const& onesub : subsets)
				{
					...
					timeTimeCalPIs = clock();
					// check if this candidate has already computed, we do not need to compute
					if (std::find(checked.begin(), checked.end(), onesub) == checked.end())
					{
						calculatePI(onesub);
						totalTimeCalPIs += double(clock() - timeTimeCalPIs);
						PkAll.insert({ onesub, PI });
...
* @retval:
*/
void classifyPattsBySize(std::map<std::string, float>& PkAll, std::map<int, std::vector<std::string>>& classPattBySize)
{
	int sizeP;
	std::map<int, std::vector<std::string>>::iterator itc;
	for (auto iter = PkAll.begin(); iter != PkAll.end(); ++iter)
	{
		sizeP = iter->first.size();
		itc = classPattBySize.find(sizeP);
		if (itc != classPattBySize.end())
		{
			itc->second.push_back(iter->first);
		}
		else
		{
			classPattBySize.insert({ sizeP, std::vector<std::string>{iter->first} });
		}
	}
}
