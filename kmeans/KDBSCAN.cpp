...
#include "../lib/Util.h"
#include <chrono>
#include "../dbscan_cpp/fuzzy_dbscan_ver1.hpp"
using namespace std;
// int eps = 5000;
int eps = 10;
// double eps = 0.03;
...
int main() {
    ...
    int choice = 0;
    // string inFileName = "/home/ankhanh/learning/data_mining/data/beijing_POI_18f.csv";
    string inFileName = "/home/ankhanh/learning/data_mining/data/sample_dataset.csv";
    string outFileName = "/home/ankhanh/learning/data_mining/output_fig/cluster_sample_dataset.txt";
    ...
    // tuples = normalization(tuples);  
    vector<Tuple<double>> means; 
    means = random_means(tuples, k);
    vector<vector<Tuple<double>>> groups = KMeans(tuples, means, ofs, dist_type, k); // 执行 KMeans 算法
    // print(clusters, ofs, k); 
    cout << "KMeans completed." << endl;

    // // const int dimen = groups[0][0].size();
    vector<vector<vector<Tuple<double>>>> clusters(k);
    vector<int> numclu(k);
    int max_num_clu = 0;
    ...
    for(int i=0; i<k; i++) {
        for(int j=0; j<numclu[i]; j++) {
            for(int l=0; l<clusters[i][j].size(); l++) {
                ...
                dis_r_cluster[i][j] = max(dis_r_cluster[i][j], distance(clusters[i][j][l], clusters[i][j][0]));
            }
        }
    }
    ...
    vector<vector<Tuple<double>>> groups2(counter);
    ...
    vector<vector<Tuple<double>>> final_clusters;
    ...
    for(int i=1; i<=counter-1; i++){
        ...
        for(int j=0; j<final_clusters.size(); j++){
            ...
            for(int l=0; l<final_clusters[j].size(); l++){
                ofs << final_clusters[j][l][0] << ' ' << final_clusters[j][l][1] << endl;
            }
        }
    }
    for(int a=0; a<k; a++) for(int c=0; c<numclu[a]; c++) if(flag[a][c] == 0){
        ...
        for(int l=0; l<clusters[a][c].size(); l++){
            ofs << clusters[a][c][l][0] << ' ' << clusters[a][c][l][1] << endl;
        }
    }
